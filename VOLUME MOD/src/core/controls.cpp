...
// ====== VOLUME STEP CONFIG (percent based) ======
#ifndef VOLUME_STEP_PERCENT
#define VOLUME_STEP_PERCENT 5  // normalny krok
#endif

#ifndef VOLUME_STEP_FAST
#define VOLUME_STEP_FAST 10    // szybkie kręcenie / long press
#endif

#define VOLUME_MAX 254

// ===== IR VOLUME MODE =====
// 0 = zawsze wolno (jak oryginał yoRadio)
// 1 = szybciej przy przytrzymaniu (repeat)
#define IR_FAST_VOLUME 1
...
static int volToPercent(int vol) {
  return (vol * 100 + VOLUME_MAX / 2) / VOLUME_MAX; // zaokrąglenie
}

static int percentToVol(int percent) {
  return (percent * VOLUME_MAX + 50) / 100;
}

static void changeVolumePercent(bool up, bool fast = false) {
  int percent = volToPercent(config.store.volume);

  int step = fast ? VOLUME_STEP_FAST : VOLUME_STEP_PERCENT;
  percent += up ? step : -step;

  if (percent < 0) percent = 0;
  if (percent > 100) percent = 100;

  int nv = percentToVol(percent);
...
#   else
    if(first){
      bool fast = abs(encoderDelta) > 1;
      //changeVolumePercent(encoderDelta > 0, fast);
      controlsEvent(encoderDelta > 0, fast ? 1 : 0);
      ...
      controlsEvent(encoderDelta > 0, 0);
    }
#   endif
  }
}
...
void irLoop() {
  ...
  if (netserver.irRecordEnable) {
    ...
    netserver.irToWs(
      typeToString(irResults.decode_type, irResults.repeat).c_str(),
      irResults.value
    );
    return;
  }
  ...
  // ==================================================
  // ===== OBSŁUGA PRZYTRZYMANIA (REPEAT)
  // ==================================================
#if IR_FAST_VOLUME
  if (irVolRepeat == 1 || irVolRepeat == 2) {
    bool up   = (irVolRepeat == 1);
    bool fast = irResults.repeat;   // repeat = szybciej

    controlsEvent(
      display.mode() == STATIONS ? !up : up,
      fast ? 1 : 0
    );
  }
#endif
  ...
  for (int target = 0; target < 17; target++) {
    for (int j = 0; j < 3; j++) {
      ...
      if (config.ircodes.irVals[target][j] != irResults.value) continue;
      ...
      switch (target) {
        ...
        // ===== GŁOŚNOŚĆ =====
        case IR_UP:
          controlsEvent(display.mode() == STATIONS ? false : true, 0);
          irVolRepeat = 1;
          break;

        case IR_DOWN:
          controlsEvent(display.mode() == STATIONS ? true : false, 0);
          irVolRepeat = 2;
          break;
...
//          controlsEvent(display.mode() == STATIONS ? false : true, 1);
//          break;
//        }
//      case 2: {
//          //controlsEvent(display.mode() == STATIONS ? true : false);
//          controlsEvent(display.mode() == STATIONS ? true : false, 1);
//          break;
//        }
//    }
//    for(int target=0; target<17; target++){
...
//} //stara koniec
/* kc-dev */
// POPRAWIONA WERSJA FUNKCJI irLoop() Z SZYBKA ZMIANA PRZY PRZYTRZYMANIU
//void irLoop() {
//  if (!irrecv.decode(&irResults)) return;
//
//  if (irResults.value < 256) return;
...
//
//  // ==================================================
//  // === OBSŁUGA PRZYTRZYMANIA VOL+/VOL− (repeat)
//  // ==================================================
//  if (irVolRepeat == 1 || irVolRepeat == 2) {
//    bool fast = irResults.repeat;
//    bool up   = (irVolRepeat == 1);
//
//    controlsEvent(
//      display.mode() == STATIONS ? !up : up,
...
//        // ====== GŁOŚNOŚĆ ======
//        case IR_UP:
//          irVolRepeat = 1;   // tylko ustawienie trybu
//          break;
//
//        case IR_DOWN:
//          irVolRepeat = 2;   // tylko ustawienie trybu
//          break;
//
//        // ====== TRYBY ======
//        case IR_HASH:
...
void onBtnDuringLongPress(int id) {
  ...
  if (checklpdelay(BTN_LONGPRESS_LOOP_DELAY, lpdelay)) {
    switch ((controlEvt_e)id) {
      case EVT_BTNLEFT: {
          controlsEvent(false, 1);
          break;
        }
      case EVT_BTNRIGHT: {
          controlsEvent(true, 1);
          break;
        }
      case EVT_BTNUP:
      case EVT_BTNDOWN: {
...
void controlsEvent(bool toRight, int8_t volDelta) {
  ...
  if (display.mode() != STATIONS) {
      ...
      display.putRequest(NEWMODE, VOL);
    #endif
    bool fast = (volDelta != 0);
    changeVolumePercent(toRight, fast);
    return;
  }
  display.resetQueue();
