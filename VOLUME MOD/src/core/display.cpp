...
void Display::_volume() {
  ...
  // 1 Pasek – SUROWA skala audio (0–254)
  if(_volbar)
    _volbar->setValue(config.store.volume);

  // 2️ Procenty – TYLKO do wyświetlania
  int vol_percent = (config.store.volume * 100 + 127) / 254;

#ifndef HIDE_VOL
  if(_voltxt) {
    char buf[8];
    snprintf(buf, sizeof(buf), "%d%%", vol_percent);
    _voltxt->setText(buf);
  }
#endif
  ...
  if(_mode == VOL) {
    timekeeper.waitAndReturnPlayer(3);
    char numbuf[8];
    snprintf(numbuf, sizeof(numbuf), "%d%%", vol_percent);
    _nums->setText(numbuf);
  }
}
